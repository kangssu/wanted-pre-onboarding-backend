...
### 1. 사용자 회원가입
* 이메일과 닉네임은 중복될 수 없기 때문에 기존에 이메일과 닉네임이 등록되어 있다면 중복되었다는 에러를 발생 시키고 회원가입이 되지 않는다.
* 비밀번호는 bcrypt 암호화 처리로 저장된다.
</br>
...
### 2. 사용자 로그인
* 회원가입이 되어있는 사용자라면 로그인이 가능하다.
* 로그인시 인증 토큰이 발급되어 인증이 필요한 곳에서 사용할 수 있다.
* 이메일과 비밀번호가 기존에 등록된 정보와 일치하지 않는다면 로그인 되지 않는다.
</br>
...
### 3. 회사 등록
* 로그인한 사용자만 회사를 등록할 수 있다.
* 회사는 사용자 아이디당 1개만 등록할 수 있고 이름이 중복되면 안된다.
* 사용자 아이디로 이미 1개의 회사가 등록되어 있거나 이름이 중복되었다면 에러를 발생 시킨다.
</br>
...
### 4. 채용공고 등록
* 로그인한 사용자만 등록이 가능하다.
* 로그인한 사용자가 회사를 등록하지 않았다면 채용공고는 등록할 수 없다.
...
### 5. 채용공고 수정
* 회사 ID를 제외하고 전부 수정되고 특정 항목 몇개만 수정할 수도 있다.
* 즉, 수정할 프로퍼티들은 선택적 프로퍼티이다.
</br>
...
### 7. 채용공고 전체 조회
* 로그인 하지 않은 사용자들도 채용공고를 전체 볼 수 있어야 한다.
* 채용공고를 등록한 회사의 정보도 같이 가져올 수 있어야 한다.
</br>
...
### 8. 채용공고 검색
* 회사 이름, 국가, 지역, 내용, 포지션, 스킬에 대해 자유롭게 검색할 수 있어야 한다.
* 1개만 검색할 수도 있고 2개 이상으로 검색할 수도 있다.
* 예를 들어 회사 이름을 주식회사로 넣는다면 회사 이름에 주식회사가 들어있는 회사들이 조회된다.
</br>
...
### 9. 채용공고 상세 조회
* 하나의 채용공고에 대해 세부적인 내용을 조회할 수 있어야 한다.
* 채용공고를 등록한 회사의 다른 채용공고의 ID 값을 가져와야 한다. 단, 현재 보고 있는 채용공고의 ID는 제외한다.
* Response 결과에서 jobVacancyAndCompany는 채용공고와 회사 정보, otherJobVacancyIds는 채용공고를 등록한 회사의 다른 채용공고로 구분해서 가져온다.
</br>
